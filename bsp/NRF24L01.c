...
/**
  ...
  * 说    明：当上层函数需要初始化时，此函数会被调用
	*           GPIO和SPI2由CubeMX生成的HAL初始化函数完成
  */
void NRF24L01_GPIO_Init(void)
{
	/* GPIO和SPI2由CubeMX生成的初始化函数完成 */
...
uint8_t NRF24L01_SPI_SwapByte(uint8_t Byte)
{
	...
	/* SPI2使用CubeMX配置的模式0、8位、高位先行进行全双工交换 */
	HAL_SPI_TransmitReceive(&hspi2, &Byte, &ReceiveByte, 1, HAL_MAX_DELAY);
...
uint8_t NRF24L01_Receive(void)
{
	...
	/*根据配置寄存器和状态寄存器判断接收状态*/
	if (Status == 0xFF || Config == 0xFF)		//SPI读回全为1，通常表示模块未连接或CSN/MISO异常
	{
		ReceiveFlag = 2;
	}
	else if ((Config & 0x02) == 0x00)		//配置寄存器位1（PWR_UP）为0
	{
